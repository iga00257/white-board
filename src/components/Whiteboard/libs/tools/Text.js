...
class PaperTextItem extends PaperItem {
  ...
  update(text) {
    return this.tool.updateText(this, Unescape(text));
  }

  updatePosition() {
    const { width = 0, height = 0, x = 0, y = 0 } = this.path.bounds;
    const fontSize = this.path.fontSize || DefaultFontSize.SMALL;
    Object.assign(this.inputbox.style, {
      left: parseInt(x || this.inputbox.style.left, 10) * this.tool.zoom + 'px',
      top: parseInt(y || this.inputbox.style.top, 10) * this.tool.zoom + 'px',
      width: parseInt(width, 10) * this.tool.zoom + 'px',
      height: parseInt(height, 10) * this.tool.zoom + 'px',
      'font-size': fontSize * this.tool.zoom + 'px',
      'line-height': fontSize * this.tool.zoom * 1.2 + 'px',
      transform: `translateY(-${(fontSize * this.tool.zoom * 1.2 - fontSize * this.tool.zoom) / 2}px)`,
    });
  }
}
...
const cursor = {
  ...
  updatePosition(instance) {
    ...
    const height = hasWrite ? path.bounds.height * ((lineIndex + 1) / lines.length) : path.bounds.height;
...
export default createTool(toolTypes.TEXT, {
  ...
  style: {
    fontFamily: '"Pingfang SC", "Microsoft YaHei", 微软雅黑, Helvetica, Arial, Lucida, Verdana, sans-serif',
    bold: 'normal',
    strokeColor: '#000000',
    italic: false,
    underline: false,
  ...
  handleTextChanged(instance, ignore = false) {
    ...
    let len = 0;
    const height = instance.inputbox.scrollHeight / scale > 50 ? instance.inputbox.scrollHeight / scale : 50;
  ...
  sendText(instance) {
    ...
    //空字符不发送
    if (!value && !instance.lastSentText) { // need to send emtpy text when user deletes a text with length from > 0 to 0
  ...
  update(instance, point, size) {
    ...
    const maxLineWidth = Math.ceil(paper.view.getTextWidth(instance.path._style.getFontStyle(), instance.lines));
    ...
    if (maxLineWidth > availableWidth) {
          ...
          if (line) {
            ...
            if(breakIdx > -1){
              ...
              if (oldSelectionStart >= breakStart) {
                ...
                instance.cursorInfo.start = oldSelectionStart === breakStart ? 0 : (oldSelectionStart - breakStart);
              }
            }
      ...
      instance.inputbox.selectionStart = instance.inputbox.selectionEnd = oldSelectionStart + offsetEnterKeys;
    }
  },
  ...
  newline(instance, line, availableWidth) {
    ...
    for (let i = 1; i <= lineLength; i++) {
      ...
      const sliceWidth = Math.ceil(paper.view.getTextWidth(instance.path._style.getFontStyle(), [sliceStr]));
      if (sliceWidth >= availableWidth) {
        if (english.test(line[i - 1])) {
          for (let m = i - 1; m > 0; m--) {
            const word = line.slice(m + 1, i - 1);
            const wordWidth = Math.ceil(paper.view.getTextWidth(instance.path._style.getFontStyle(), [word]));
